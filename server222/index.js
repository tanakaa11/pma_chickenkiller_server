...
const loginSchema = z.object({ email, password: pwd });
...
const updatePasswordSchema = z.object({ email, newPassword: pwd });
...
const emailTransporter = (process.env.SMTP_USER && process.env.SMTP_APP_PASS)
  ? nodemailer.createTransport({
      service: 'gmail',
      auth: { user: process.env.SMTP_USER, pass: process.env.SMTP_APP_PASS },
    })
  : null;
...
// ============================================================================
// CORS + HELMET — single source of truth, no competing handlers
// ============================================================================
...
// helmet first — disable every policy that touches cross-origin headers
app.use(helmet({
...
// Single cors() instance handles both preflight OPTIONS and actual requests
...
app.options('*', corsMiddleware); // handle preflight for every route
...
const hashPassword = async (plainPassword) => {
  try { return await bcrypt.hash(plainPassword, SALT_ROUNDS); }
  catch (error) { console.error('Password hashing error:', error); throw new Error('Failed to hash password'); }
};

const verifyPassword = async (plainPassword, hashedPassword) => {
  try { return await bcrypt.compare(plainPassword, hashedPassword); }
  catch (error) { console.error('Password verification error:', error); return false; }
};
...
const loginHandler = async (req, res) => {
  ...
  if (!u.password) return res.status(401).json(err('Invalid email or password'));
  ...
  if (!passwordValid) return res.status(401).json(err('Invalid email or password'));
  ...
  await supabase.from('refresh_tokens').insert({
    user_id: user.id, token_hash: hashToken(refreshToken),
...
const refreshHandler = async (req, res) => {
  ...
  await supabase.from('refresh_tokens').insert({
    user_id: u.id, token_hash: hashToken(newRefreshToken),
    expires_at: Date.now() + REFRESH_TTL_MS,
  });
...
const meHandler = async (req, res) => {
  ...
  const { data: u } = await supabase.from('users').select(USER_SELECT).eq('id', payload.userId).single();
...
app.get('/pma/users/check-email', async (req, res) => {
  ...
  const { data: user } = await supabase
    .from('users').select('id').eq('email', String(email).toLowerCase()).maybeSingle();
  // Never return the full user object on a public endpoint
...
app.post('/pma/users/:id/link-practice', async (req, res) => {
  ...
  if (roleId) {
    const ROLE_NAMES = { 'ROLE_SYSADMIN': 'SystemAdministrator', 'ROLE_ADMIN': 'PracticeAdministrator', 'ROLE_PRACTITIONER': 'PracticePractitioner' };
  ...
  res.json(success({ user: formatUser(updatedUser), otp, token: randomUUID() }, 'Practice linked and OTP generated'));
});
...
app.post('/pma/admin/link-user-to-practice', async (req, res) => {
  ...
  if (!user) {
    ...
    const { error: createErr } = await supabase.from('users').insert({
      id: newId, email, first_name: firstName, last_name: lastName,
      is_active: true, role: roleUiMap[roleId], password: null,
    });
  ...
  const { data: alreadyLinked } = await supabase.from('user_practices')
    .select('id').eq('user_id', user.id).eq('practice_id', practiceId).maybeSingle();
  if (!alreadyLinked) {
    await supabase.from('user_practices').insert({ user_id: user.id, practice_id: practiceId, practice_name: practice.name });
  }
  ...
  if (roleId === 'ROLE_PRACTITIONER') {
    ...
    if (!existingDoctor) {
      await supabase.from('doctors').insert({
        id: randomUUID(), user_id: user.id, first_name: user.first_name, last_name: user.last_name,
        specialization: 'General Practice', email: user.email, phone: '', is_available: true,
      });
    }
  }
...
app.post('/pma/admin/send-otp', async (req, res) => {
  ...
  const emailed = !!emailTransporter;
...
app.post('/pma/admin/create-and-invite', async (req, res) => {
  ...
  const { error: uErr } = await supabase.from('users').insert({
    ...
    password: hashedPw, role: roleUiMap[roleId] || 'reception', is_active: false,
  });
  ...
  if (roleId === 'ROLE_PRACTITIONER') {
    await supabase.from('doctors').insert({
      id: randomUUID(), user_id: newId, first_name: firstName, last_name: lastName,
...
app.post('/pma/practices/verify-otp', async (req, res) => {
  ...
  await supabase.from('otp_tokens').delete().eq('id', stored.id);
  res.json(success({ practiceId: practice.id, practiceName: practice.name, practiceNumber: practice.practice_number, verified: true }, 'OTP verified and practice linked successfully'));
});
...
app.post('/pma/patients', async (req, res) => {
  ...
  const { error: insertErr } = await supabase.from('patients').insert({
    ...
    phone: data.phone, email: data.email, practice_id: data.practiceId,
...
app.post('/pma/patients/:id/beneficiaries', async (req, res) => {
  ...
  if (bData.address) {
    await supabase.from('patient_addresses').insert({ patient_id: newId, street: bData.address.street, city: bData.address.city, province: bData.address.province, postal_code: bData.address.postalCode });
  }
  await supabase.from('beneficiaries').insert({ id: randomUUID(), patient_id: newId, main_member_id: req.params.id, relationship });
  const { data: newPatient } = await supabase.from('patients').select(PATIENT_SELECT).eq('id', newId).single();
...
app.get('/pma/appointments', async (req, res) => {
  ...
  if (!isSuperSuperAdmin || practiceId) {
    const { data: practitionerIds } = await supabase.from('practice_practitioners').select('user_id').eq('practice_id', practiceId);
    const { data: doctorIds }       = await supabase.from('doctors').select('id').in('user_id', (practitionerIds || []).map(p => p.user_id));
    if (doctorIds?.length > 0) { query = query.in('doctor_id', doctorIds.map(d => d.id)); }
    else { return res.json(success([])); }
  }

  if (status) { const statuses = Array.isArray(status) ? status : [status]; query = statuses.length === 1 ? query.eq('status', statuses[0]) : query.in('status', statuses); }
  ...
  const stale = (appointments || []).filter(a => a.date < today && (a.status === 'in_consultation' || a.status === 'confirmed'));
  if (stale.length > 0) {
    const staleIds = stale.map(a => a.id);
    await supabase.from('appointments').update({ status: 'completed', updated_at: new Date().toISOString() }).in('id', staleIds);
    for (const a of appointments) { if (staleIds.includes(a.id)) a.status = 'completed'; }
  }
...
app.post('/pma/appointments', async (req, res) => {
  ...
  const calculateEnd = (s) => { const [h, m] = s.split(':').map(Number); const em = m + 30; return `${String(h + Math.floor(em / 60)).padStart(2, '0')}:${String(em % 60).padStart(2, '0')}`; };
  const newId = randomUUID();
  const now   = new Date().toISOString();
...
// ============================================================================
// PRACTICE INFO ENDPOINTS
// ============================================================================

const enrichPP = (pp, usersMap) => {
  const u = usersMap?.[pp.user_id] || {};
  return { ...toCamel(pp), practitioner: { id: pp.user_id, firstName: u.first_name || '', lastName: u.last_name || '', email: u.email || '', hpcsaNumber: pp.hpcsa_number || '' } };
};
...
app.get('/pma/practice', async (req, res) => {
  ...
  const usersMap = Object.fromEntries((allUsers || []).map(u => [u.id, u]));
  const memberMap = {};
  for (const r of (roleRows || [])) { if (!memberMap[r.user_id]) memberMap[r.user_id] = { userId: r.user_id, roleId: r.role_id, roleName: r.role_name }; }
  for (const pp of (ppRows || [])) { if (!memberMap[pp.user_id]) memberMap[pp.user_id] = { userId: pp.user_id, roleId: 'ROLE_PRACTITIONER', roleName: 'PracticePractitioner' }; }
  ...
  res.json(success({ ...toCamel(practice), practicePractitioners: (practice.practice_practitioners || []).map(pp => enrichPP(pp, usersMap)), patients: (patients || []).map(toCamel), linkedUsers }));
});
...
app.post('/pma/visits', async (req, res) => {
  ...
  if (data.vitals) { const { error: vErr } = await supabase.from('visit_vitals').insert({ ...snakeKeys(data.vitals), id: randomUUID(), visit_id: visitId }); if (vErr) console.error('vitals insert error:', vErr.message); }
  if (data.diagnoses?.length) { const { error: dErr } = await supabase.from('visit_diagnoses').insert(data.diagnoses.map(d => ({ id: randomUUID(), ...snakeKeys(d), visit_id: visitId }))); if (dErr) console.error('diagnoses insert error:', dErr.message); }
  if (data.procedures?.length) { const { error: pErr } = await supabase.from('visit_procedures').insert(data.procedures.map(p => ({ id: randomUUID(), ...snakeKeys(p), visit_id: visitId }))); if (pErr) console.error('procedures insert error:', pErr.message); }
...
app.get('/pma/patients/:id/clinical-record', async (req, res) => {
  const { data: visits } = await supabase.from('visits').select(VISIT_SELECT).eq('patient_id', req.params.id).order('visit_date', { ascending: false });
  const enriched = await enrichVisitsBatch((visits || []).map(formatVisit));
...
app.get('/pma/invoices', async (req, res) => {
  ...
  // Scope invoices to practice via patient ownership
...
// ============================================================================
// OTP ENDPOINTS (appointment booking — guests)
// ============================================================================
...
app.post('/pma/otp/verify', otpVerifyLimiter, async (req, res) => {
  ...
  if (!stored) { for (const [k, v] of otpStore.entries()) { if ((email && v.resolvedEmail === email) || (phone && k === phone)) { key = k; stored = v; break; } } }
  ...
  if (stored.code === code) {
    ...
    if (appointmentData) {
      try {
        ...
        const { data: conflict } = await supabase.from('appointments').select('id').eq('doctor_id', d.doctorId).eq('date', d.date).eq('start_time', d.startTime).not('status', 'in', '("cancelled","rejected")').maybeSingle();
        if (conflict) return res.status(400).json(err('This time slot is already booked'));
        const { data: patientDayConflict } = await supabase.from('appointments').select('id').eq('patient_id', d.patientId).eq('date', d.date).not('status', 'in', '("cancelled","rejected")').maybeSingle();
        if (patientDayConflict) return res.status(400).json(err('This patient already has an appointment booked for this date.'));
        const calculateEnd = (s) => { const [h, m] = s.split(':').map(Number); const em = m + 30; return `${String(h + Math.floor(em / 60)).padStart(2, '0')}:${String(em % 60).padStart(2, '0')}`; };
        const newId = randomUUID();
        const now = new Date().toISOString();
...
// ============================================================================
// PASSWORD UPDATE — requires current password for verification
// ============================================================================
